...
<div class="bg-white rounded-lg shadow-md p-6 mb-6">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center">
                    ...
                    <span class="px-2 py-1 rounded text-sm font-medium 
                        ...
                        <?php 
                        $typeIndo = $playbook['type'];
                        if ($typeIndo == 'Automated') $typeIndo = 'Otomatis';
                        elseif ($typeIndo == 'Manual') $typeIndo = 'Manual';
                        elseif ($typeIndo == 'Semi-Automated') $typeIndo = 'Semi-Otomatis';
                        echo esc($typeIndo);
                        ?>
                    </span>
                </div>
                <div class="flex items-center">
                    ...
                    <span class="px-2 py-1 rounded text-sm font-medium 
                        ...
                        <?php 
                        $sevIndo = $playbook['severity_level'];
                        if ($sevIndo == 'Critical') $sevIndo = 'Kritis';
                        elseif ($sevIndo == 'High') $sevIndo = 'Tinggi';
                        elseif ($sevIndo == 'Medium') $sevIndo = 'Sedang';
                        elseif ($sevIndo == 'Low') $sevIndo = 'Rendah';
                        echo esc($sevIndo);
                        ?>
                    </span>
                </div>
        ...
        <div class="space-y-3">
            <div class="flex items-center">
                <span class="font-medium text-gray-700 w-32">Est. Waktu:</span>
                <span class="text-gray-900"><?= esc($playbook['estimated_time']) ?></span>
            </div>
            <div class="flex items-center">
                <span class="font-medium text-gray-700 w-32">Dibuat Oleh:</span>
                <span class="text-gray-900"><?= esc($playbook['created_by'] ?? 'Sistem') ?></span>
            </div>
            <div class="flex items-center">
                <span class="font-medium text-gray-700 w-32">Dibuat Pada:</span>
                <span class="text-gray-900"><?= date('j M Y, H:i', strtotime($playbook['created_at'])) ?></span>
            </div>
        </div>
    </div>
    ...
    <div class="border-t border-gray-200 pt-6 mt-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-3">Alat yang Dibutuhkan</h3>
        <p class="text-gray-700"><?= esc($playbook['required_tools']) ?></p>
    </div>
</div>

<div class="bg-white rounded-lg shadow-md p-6">
    <h3 class="text-lg font-semibold text-gray-800 mb-4">Langkah Eksekusi</h3>
    ...
    <?php if (!empty($playbook['steps'])): ?>
                    ...
                    <div class="flex items-start border border-gray-200 rounded-lg p-4">
                        ...
                        <div class="flex-1">
                            ...
                            <?php if (!empty($step['estimated_time'])): ?>
                                <p class="text-sm text-gray-600 mt-1">Est. waktu: <?= esc($step['estimated_time']) ?></p>
                            <?php endif; ?>
                        </div>
        ...
        <?php endif; ?>
    <?php else: ?>
        <p class="text-gray-500 italic">Tidak ada langkah yang ditentukan untuk buku panduan ini.</p>
    <?php endif; ?>
    ...
    <div class="mt-6 flex justify-between items-center">
        <div>
            <label class="inline-flex items-center">
                ...
                <span class="ml-2 text-sm text-gray-600">Saya mengonfirmasi bahwa saya telah meninjau semua langkah dan siap untuk menjalankan buku panduan ini</span>
            </label>
        </div>
        <button id="execute_btn" disabled class="bg-green-500 text-white font-bold py-2 px-4 rounded opacity-50 cursor-not-allowed">
            <i class="fas fa-play"></i> Jalankan Buku Panduan
        </button>
    </div>
</div>
...
document.getElementById('execute_btn').addEventListener('click', function() {
    if (confirm('Apakah Anda yakin ingin menjalankan buku panduan ini?')) {
        // In a real implementation, this would trigger the actual playbook execution
        alert('Pelaksanaan buku panduan dimulai. Dalam implementasi nyata, ini akan memicu langkah-langkah otomatis.');
        ...
        .then(data => {
            if (data.success) {
                ...
                alert('Kesalahan saat menjalankan buku panduan: ' + data.message);
            }
        })
        .catch(error => {
            console.error('Error:', error);
            alert('Terjadi kesalahan saat menjalankan buku panduan.');
        });
    }
});
</script>

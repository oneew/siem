                ...
                <div class="bg-indigo-50 px-6 py-4 border-b border-indigo-200">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            $statusIndo = $case['status'];
                            switch($case['status']) {
                                case 'Active': $statusIndo = 'Aktif'; break;
                                case 'In Progress': $statusIndo = 'Sedang Berlangsung'; break;
                                case 'On Hold': $statusIndo = 'Ditunda'; break;
                                case 'Completed': $statusIndo = 'Selesai'; break;
                                case 'Archived': $statusIndo = 'Diarsipkan'; break;
                            }
                            
                            $prioIndo = $case['priority'];
                            switch($case['priority']) {
                                case 'Critical': $prioIndo = 'Kritis'; break;
                                case 'High': $prioIndo = 'Tinggi'; break;
                                case 'Medium': $prioIndo = 'Sedang'; break;
                                case 'Low': $prioIndo = 'Rendah'; break;
                            }
                            ?>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div class="text-center p-4 bg-blue-50 rounded-lg">
                            <div class="text-2xl font-bold text-blue-600 mb-1">
                                <?php
                                $typeIndo = $case['case_type'];
                                switch($case['case_type']) {
                                    case 'Malware Analysis': $typeIndo = 'Analisis Malware'; break;
                                    case 'Network Forensics': $typeIndo = 'Forensik Jaringan'; break;
                                    case 'Disk Forensics': $typeIndo = 'Forensik Disk'; break;
                                    case 'Mobile Forensics': $typeIndo = 'Forensik Seluler'; break;
                                    case 'Memory Forensics': $typeIndo = 'Forensik Memori'; break;
                                    case 'Email Forensics': $typeIndo = 'Forensik Email'; break;
                                    case 'Database Forensics': $typeIndo = 'Forensik Database'; break;
                                    case 'Cloud Forensics': $typeIndo = 'Forensik Cloud'; break;
                                }
                                echo esc($typeIndo);
                                ?>
                            </div>
                            <div class="text-sm text-gray-600">Tipe Kasus</div>
                        </div>
                        
                        <div class="text-center p-4 bg-green-50 rounded-lg">
                            <div class="text-2xl font-bold text-green-600 mb-1"><?= $case['evidence_count'] ?: 0 ?></div>
                            <div class="text-sm text-gray-600">Item Bukti</div>
                        </div>
                        ...
                        <div class="text-center p-4 bg-orange-50 rounded-lg">
                            ...
                            <div class="text-sm text-gray-600">Penyelidik</div>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                        ...
                        <div class="p-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-500 mb-1">Penyelidik yang Ditugaskan</label>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-500 mb-1">Tanggal Insiden</label>
                                    <p class="text-gray-900">
                                        <?= $case['incident_date'] ? date('j M Y, H:i', strtotime($case['incident_date'])) : 'Tidak ditentukan' ?>
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 px-6 py-4 border-b border-gray-200">
                            <div class="flex justify-between items-center">
                                <h3 class="text-lg font-semibold text-gray-900 flex items-center">
                                    ...
                                    Lacak Balak (Chain of Custody) Bukti
                                </h3>
                            ...
                            <div id="evidenceList" class="space-y-4">
                                ...
                                <div class="border border-gray-200 rounded-lg p-4 bg-gray-50">
                                    <div class="flex justify-between items-start">
                                        <div class="flex-1">
                                            <h4 class="font-medium text-gray-900">Dump Memori Sistem</h4>
                                            <p class="text-sm text-gray-600 mt-1">Citra RAM lengkap yang diambil dari workstation yang terdampak</p>
                                ...
                                <div class="border border-gray-200 rounded-lg p-4 bg-gray-50">
                                    <div class="flex justify-between items-start">
                                        <div class="flex-1">
                                            <h4 class="font-medium text-gray-900">Tangkapan Paket Jaringan</h4>
                                            <p class="text-sm text-gray-600 mt-1">File PCAP berisi lalu lintas jaringan yang mencurigakan</p>
                                            <div class="flex items-center space-x-4 mt-2 text-xs text-gray-500">
                                ...
                                <div class="text-center py-8 text-gray-500" id="noEvidence" style="display: none;">
                                    ...
                                    <p>Belum ada item bukti yang tercatat</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                        ...
                        <div class="p-6 space-y-3">
                            ...
                            <button onclick="assignInvestigator()" class="w-full bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg text-sm transition-colors">
                                <i class="fas fa-user-plus mr-2"></i>Tugaskan Penyelidik
                            </button>
                        ...
                        <div class="p-6">
                            <div class="space-y-3">
                                ...
                                <div class="text-center py-4 text-gray-500 text-sm" id="noRelated" style="display: none;">
                                    Tidak ada kasus terkait ditemukan
                                </div>
                            </div>
                        </div>
                    </div>
...
function updateStatus() {
    const newStatus = prompt('Masukkan status baru (Active, In Progress, On Hold, Completed, Archived):');
    if (newStatus) {
        alert(`Status kasus diperbarui menjadi: ${newStatus} (Mode Demo)`);
        // In production: make API call to update status
    }
}

function assignInvestigator() {
    const investigator = prompt('Masukkan nama penyelidik:');
    if (investigator) {
        alert(`Kasus ditugaskan kepada: ${investigator} (Mode Demo)`);
        // In production: make API call to assign investigator
    }
}

function generateReport() {
    alert('Membuat laporan forensik... (Mode Demo)\n\nLaporan akan mencakup ringkasan kasus, analisis bukti, dan rekomendasi.');
    // In production: redirect to report generation
}

function closeCase() {
    if (confirm('Apakah Anda yakin ingin menutup kasus ini? Tindakan ini tidak dapat dibatalkan.')) {
        alert('Kasus ditandai sebagai ditutup (Mode Demo)');
        // In production: make API call to close case
    }
}

function addEvidence() {
    const evidenceName = prompt('Masukkan nama item bukti:');
    if (evidenceName) {
        ...
        newEvidence.innerHTML = `
            <div class="flex justify-between items-start">
                <div class="flex-1">
                    ...
                    <p class="text-sm text-gray-600 mt-1">Item bukti baru ditambahkan</p>
                    <div class="flex items-center space-x-4 mt-2 text-xs text-gray-500">
                        <span><i class="fas fa-calendar mr-1"></i>Dikumpulkan: ${new Date().toLocaleString()}</span>
                        <span><i class="fas fa-shield-alt mr-1"></i>Hash: Tertunda</span>
                    </div>
                </div>
                <span class="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded">Tertunda</span>
            </div>
        `;
        evidenceList.appendChild(newEvidence);
        alert('Item bukti ditambahkan (Mode Demo)');
    }
}

function exportCase(format) {
    if (format === 'pdf') {
        alert('Mengekspor kasus ke format PDF (Mode Demo)\n\nLaporan akan mencakup semua detail kasus, linimasa, dan lacak balak bukti.');
